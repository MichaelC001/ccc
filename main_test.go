...
func TestDBAppendAndQuery(t *testing.T) {
	...
	// Append a message (delivered to Telegram)
	rec := &MessageRecord{
		ID:      "test:1",
		Session: session,
		Type:    "user_prompt",
	...
	// isDelivered check
	if !isDelivered("test:1") {
		t.Error("should be delivered")
	}

	// Dedup: ON CONFLICT should not downgrade delivery
	...
	if !isDelivered("test:1") {
		t.Error("dedup failed: delivery was downgraded")
	}
	...
	if pending[0].ID != "test:2" {
		t.Errorf("pending ID = %q, want test:2", pending[0].ID)
	}

	// markDelivered
	markDelivered("test:2", 12345)
	if !isDelivered("test:2") {
		t.Error("should be delivered after markDelivered")
	}
	pending = findPending(session)
	if len(pending) != 0 {
		t.Fatalf("findPending returned %d after markDelivered, want 0", len(pending))
	}

	// isFromTelegram
	if !isFromTelegram(session, "hello world") {
		t.Error("isFromTelegram should match existing message")
	}
	if isFromTelegram(session, "nonexistent") {
		t.Error("isFromTelegram should not match nonexistent message")
	}

	// logEvent
	logEvent(session, "test_event", "test", "test:1", "test detail")
	// Just verify no crash — events are append-only for debugging
}
...
func TestAllSessions(t *testing.T) {
	...
	if len(sessions) != 0 {
		t.Fatalf("allSessions returned %d, want 0", len(sessions))
	}

	// Add pending message
	appendMessage(&MessageRecord{
		ID: "test:1", Session: "sess-a", Type: "assistant_text", Text: "hi", Origin: "claude",
	})
	appendMessage(&MessageRecord{
		ID: "test:2", Session: "sess-b", Type: "notification", Text: "yo", Origin: "claude",
	})
	sessions = allSessions()
	if len(sessions) != 2 {
		t.Fatalf("allSessions returned %d, want 2", len(sessions))
	}
}
...
func TestMarkdownToHTML(t *testing.T) {
	tests := []struct {
		...
		{"bold underscores", "__bold__", "<b>bold</b>"},
		{"italic stars", "*italic*", "<i>italic</i>"},
		{"strikethrough", "~~deleted~~", "<s>deleted</s>"},
		{"inline code", "use `fmt.Println`", "use <code>fmt.Println</code>"},
		{"inline code html", "use `a<b>c`", "use <code>a&lt;b&gt;c</code>"},
		{"code block", "```go\nfmt.Println(\"hi\")\n```", "<pre>fmt.Println(\"hi\")</pre>"},
		{"code block html escape", "```\na < b\n```", "<pre>a &lt; b</pre>"},
		{"link", "[Google](https://google.com)", `<a href="https://google.com">Google</a>`},
		{"mixed", "**bold** and `code`", "<b>bold</b> and <code>code</code>"},
		{"no markdown", "just plain text 🎉", "just plain text 🎉"},
	}
