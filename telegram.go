...
// sendMessageGetID sends a message (converting Markdown to HTML) and returns the message ID
func sendMessageGetID(config *Config, chatID int64, threadID int64, text string) (int64, error) {
	return sendMessageWithMode(config, chatID, threadID, markdownToHTML(text), "HTML")
}

// sendMessageHTMLGetID sends a message with HTML parse mode (no conversion) and returns the message ID
...
// editMessage edits an existing message (converting Markdown to HTML), sending overflow as new messages
func editMessage(config *Config, chatID int64, messageID int64, threadID int64, text string) error {
	return editMessageWithMode(config, chatID, messageID, threadID, markdownToHTML(text), "HTML")
}
...
// markdownToHTML converts Markdown text to Telegram-supported HTML subset.
// Handles: code blocks, inline code, bold, italic, strikethrough, links.
// Plain text (no markdown) passes through safely with HTML escaping.
func markdownToHTML(text string) string {
	...
	// Extract inline code to protect from other conversions
	var inlineCode []string
	inlinePlaceholder := "\x00INLINECODE%d\x00"
	inlineCodeRe := regexp.MustCompile("`([^`]+)`")
	text = inlineCodeRe.ReplaceAllStringFunc(text, func(match string) string {
		parts := inlineCodeRe.FindStringSubmatch(match)
		inlineCode = append(inlineCode, parts[1])
		return fmt.Sprintf(inlinePlaceholder, len(inlineCode)-1)
	})
	...
	for i, code := range inlineCode {
		...
		escaped = strings.ReplaceAll(escaped, ">", "&gt;")
		text = strings.Replace(text, fmt.Sprintf(inlinePlaceholder, i), "<code>"+escaped+"</code>", 1)
	}

	// Restore code blocks (with HTML escaping)
	for i, block := range blocks {
		escaped := strings.ReplaceAll(block.code, "&", "&amp;")
		escaped = strings.ReplaceAll(escaped, "<", "&lt;")
		escaped = strings.ReplaceAll(escaped, ">", "&gt;")
		escaped = strings.TrimRight(escaped, "\n")
		text = strings.Replace(text, fmt.Sprintf(placeholder, i), "<pre>"+escaped+"</pre>", 1)
	}
